...
echo "Template path: $CLOUDFRONT_TEMPLATE"
echo "Change set name: $CHANGE_SET_NAME"

# Validate the template
if ! cfn-lint -t "$CLOUDFRONT_TEMPLATE"; then
...
CHANGE_SET_ARN=$(aws cloudformation deploy \
    ...
    --template-file "$CLOUDFRONT_TEMPLATE" \
...
then
    ...
    SECONDS=0
    while true; do
        STATUS=$(aws cloudformation describe-stacks --stack-name "$CLOUDFRONT_STACK" --query 'Stacks[0].StackStatus' --output text)
